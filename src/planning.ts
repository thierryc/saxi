...
export const XylodrawFast: ToolingProfile = {
  penDownProfile: {
    acceleration: 200 * Device.Xylodraw.stepsPerMm,
    maximumVelocity: 50 * Device.Xylodraw.stepsPerMm,
    corneringFactor: 0.127 * Device.Xylodraw.stepsPerMm
  },
  penUpProfile: {
    acceleration: 400 * Device.Xylodraw.stepsPerMm,
    maximumVelocity: 200 * Device.Xylodraw.stepsPerMm,
    corneringFactor: 0
  },
  penUpPos: Device.Xylodraw.penPctToPos(50),
  penDownPos: Device.Xylodraw.penPctToPos(60),
  penDropDuration: 0.12,
  penLiftDuration: 0.12,
  deviceName: 'Xylodraw',
};
...
export class Plan {
  ...
  public withPenHeights(penUpHeight: number, penDownHeight: number, planOptions: PlanOptions): Plan {
    ...
    return new Plan(this.motions.map((motion, j) => {
      if (motion instanceof XYMotion) {
        ...
        if (j === this.motions.length - 3) {
          return new PenMotion(penDownHeight, Device[planOptions.deviceName].penPctToPos(0), motion.duration());
        } else if (j === this.motions.length - 1) {
          return new PenMotion(Device[planOptions.deviceName].penPctToPos(0), penUpHeight, motion.duration());
        }
...
export function plan(
  ...
  motions.push(new PenMotion(Device[profile.deviceName].penPctToPos(penMaxUpPos), profile.penUpPos, profile.penDropDuration));
